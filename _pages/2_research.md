...
.publications .abstract.btn:focus,
.publications .abstract.btn:active {
    background: #0056b3 !important;
...
html[data-theme='dark'] .publications .abstract.btn {
    background: #0d6efd !important;
    border-color: #0d6efd !important;
    color: #ffffff !important;
}

html[data-theme='dark'] .publications .abstract.btn:hover {
    background: #0b5ed7 !important;
    border-color: #0b5ed7 !important;
}

html[data-theme='dark'] .publications a.btn[href*="doi"],
html[data-theme='dark'] .publications a.btn[href*="ieee"],
html[data-theme='dark'] .publications a.btn[href*="springer"],
html[data-theme='dark'] .publications a.btn[href*="acm"] {
    border-color: #198754;
    color: #198754;
}

html[data-theme='dark'] .publications a.btn[href*="doi"]:hover,
html[data-theme='dark'] .publications a.btn[href*="ieee"]:hover,
html[data-theme='dark'] .publications a.btn[href*="springer"]:hover,
html[data-theme='dark'] .publications a.btn[href*="acm"]:hover {
    background: #198754;
    border-color: #198754;
    color: #ffffff;
}
...
.publications .abstract:not(.btn):not(.hidden) {
    display: block;
}
...
/* Hide any stray HTML tags - more aggressive */
.publications:after,
.publications ol:after,
.publications ol.bibliography:after,
.publications .bibliography:after {
    content: '' !important;
    display: none !important;
...
<script>
// Abstract toggle functionality - fixed version
document.addEventListener('DOMContentLoaded', function() {
    console.log('Research page loaded, initializing abstract buttons...');
    
    // Add active state styling inside DOMContentLoaded
    ...
    style.textContent = `
        ...
        html[data-theme='dark'] .publications .abstract.btn.active {
            background: #0b5ed7 !important;
            border-color: #0b5ed7 !important;
        }
    `;
    ...
    // Function to setup a single abstract button
    function setupAbstractButton(button) {
        ...
        button.addEventListener('click', function(e) {
            ...
            // Find the abstract div
            let abstractDiv = null;
            
            // Look in the same list item
            const listItem = this.closest('li');
            if (listItem) {
                abstractDiv = listItem.querySelector('.abstract:not(.btn)');
            }
            
            // If not found, look in parent's siblings
            if (!abstractDiv) {
                let sibling = this.parentElement;
                while (sibling && !abstractDiv) {
                    sibling = sibling.nextElementSibling;
                    if (sibling && sibling.classList && sibling.classList.contains('abstract') && !sibling.classList.contains('btn')) {
                        abstractDiv = sibling;
                        break;
                    }
                }
    ...
    // Reinitialize after a short delay for any dynamic content
    setTimeout(initializeAbstractButtons, 500);
});
</script>
