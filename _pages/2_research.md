...
<script>
// Abstract toggle functionality - enhanced version
document.addEventListener('DOMContentLoaded', function() {
    // Wait a moment for any dynamic content to load
    setTimeout(function() {
        const abstractButtons = document.querySelectorAll('.abstract.btn');
        console.log('Found abstract buttons:', abstractButtons.length);
        
        abstractButtons.forEach((button, index) => {
            ...
            newButton.addEventListener('click', function(e) {
                ...
                // Find the abstract div within the same parent structure
                let abstractDiv = null;
                
                // Try finding in the parent element first
                const parentDiv = this.parentElement;
                abstractDiv = parentDiv.querySelector('.abstract:not(.btn)');
                
                // If not found, try the grandparent (in case of nested structure)
                if (!abstractDiv) {
                    const grandParent = parentDiv.parentElement;
                    abstractDiv = grandParent.querySelector('.abstract:not(.btn)');
                }
                
                // If still not found, try the closest li
                if (!abstractDiv) {
                    const parentLi = this.closest('li');
                    if (parentLi) {
                        abstractDiv = parentLi.querySelector('.abstract:not(.btn)');
                    }
                }
                
                console.log('Found abstract div:', abstractDiv);
                ...
                if (abstractDiv) {
                    ...
                    console.error('Abstract div not found');
                }
            });
        });
    }, 500); // Increased timeout for safety
    
    // Remove any visible HTML tags
    const bodyText = document.body.innerHTML;
    if (bodyText.includes('&lt;/ol&gt;') || bodyText.includes('</ol>')) {
        document.body.innerHTML = bodyText.replace(/&lt;\/ol&gt;/g, '').replace(/&lt;\/li&gt;/g, '');
    }
    
    // Find and hide any text nodes containing HTML tags
    ...
    while (walker.nextNode()) {
        ...
        if (node.nodeValue && (node.nodeValue.includes('</ol>') || node.nodeValue.includes('</li>'))) {
            node.nodeValue = node.nodeValue.replace(/<\/ol>/g, '').replace(/<\/li>/g, '');
        }
    }
});
</script>
